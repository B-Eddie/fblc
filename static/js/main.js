...
document.addEventListener("DOMContentLoaded", function () {
  // Initialize global variables rather than redeclaring them
  ...
  // Initialize map - use the global variable
  ...
  // Initialize user location but don't redeclare providers
  let userLocation = { lat: 37.7749, lng: -122.4194 };

  // Set up filter button event listeners with improved specialty handling
  filterButtons.forEach((button) => {
    button.addEventListener("click", () => {
      const specialtyWithDashes = button.id.replace("filter", "");
      const specialty = specialtyWithDashes.replace(/-/g, " ");
